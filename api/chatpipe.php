...
if(!$cfg->enableEmailPiping())
    api_exit(EX_UNAVAILABLE,translate('ERROR_SERVICES_NOT_AVAILABLE'));
//Get the input
...
if(!$parser->decode()){ //Decode...returns false on decoding errors
    api_exit(EX_NOINPUT,translate('ERROR_EMAIL_PARSE_FAILED').'['.$parser->getError()."]\n\n".$data);    
}
...
if(!$fromlist || PEAR::isError($fromlist)){
    api_exit(EX_DATAERR,translate('ERROR_INVALID_FROM_ADDRESS').' ['.$fromlist->getMessage()."]\n\n".$data);
}
...
if(!$ticket){ //New tickets...
    ...
    if(!is_object($ticket) || $errors){
        api_exit(EX_DATAERR,translate('TEXT_CREATE_FAILED').' '.implode("\n",$errors)."\n\n");
    }
