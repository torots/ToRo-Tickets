...
?>
<div class="msg"><?php print translate('LABEL_DEPARTMENTS')?></div>
<table width="100%" border="0" cellspacing=1 cellpadding=2>
    ...
    <table border="0" cellspacing=0 cellpadding=2 class="dtable" align="center" width="100%">
        <tr>
	        <th width="7px">&nbsp;</th>
	        <th><?php print translate('LABEL_DEPT_NAME')?></th>
            <th><?php print translate('LABEL_TYPE')?></th>
            <th width=10><?php print translate('LABEL_USERS')?></th>
            <th><?php print translate('LABEL_PRIMARY_OUTGOING_EMAIL')?></th>
            <th><?php print translate('LABEL_MANAGER')?></th>
        </tr>
        <?
        $class = 'row1';
        $total=0;
        ...
        if($depts && db_num_rows($depts)):
            ...
            while ($row = db_fetch_array($depts)) {
                ...
                $default=($defaultId==$row['dept_id'])?'('.translate('LABEL_DEFAULT').')':'';
                ?>
            <tr class="<?=$class?>" id="<?=$row['dept_id']?>">
                ...
                <td><?=$row['ispublic']?translate('LABEL_PUBLIC'):'<b>'.translate('LABEL_PUBLIC').'</b>'?></td>
                <td>&nbsp;&nbsp;
                    <b>
                    <?if($row['users']>0) {?>
            ...
            <tr class="<?=$class?>"><td colspan=6><b><?php print translate('TEXT_QUERY_RETURNED_ZERO_RESULTS')?></b></td></tr>
        <?
        endif; ?>
    </table>
    </td></tr>
    ...
    <tr>
        <td style="padding-left:20px">
            Select:&nbsp;
            <a href="#" onclick="return select_all(document.forms['depts'],true)"><?php print translate('LABEL_ALL')?></a>&nbsp;&nbsp;
            <a href="#" onclick="return reset_all(document.forms['depts'])"><?php print translate('LABEL_NONE')?></a>&nbsp;&nbsp;
            <a href="#" onclick="return toogle_all(document.forms['depts'],true)"><?php print translate('LABEL_TOGGLE')?></a>&nbsp;&nbsp;
        </td>
    </tr>
    <tr>
        <td align="center">
            <input class="button" type="submit" name="public" value="<?php print translate('LABEL_MAKE_PUBLIC')?>"
                onClick=' return confirm("Are you sure you want to make selected depts(s) public?");'>
            <input class="button" type="submit" name="private" value="<?php print translate('LABEL_MAKE_PRIVATE')?>" 
                onClick=' return confirm("Are you sure you want to make selected depts(s) private?");'>
            <input class="button" type="submit" name="delete" value="<?php print translate('LABEL_DELETE_DEPT_S')?>" 
