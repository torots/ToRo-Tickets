...
if(!defined('OSTSCPINC') || !is_object($ticket) || !is_object($thisuser) || !$thisuser->isStaff()) die(translate("TEXT_ACCESS_DENIED"));
...
<table width="100%" border="0" cellspacing=1 cellpadding=2>
  <form action="tickets.php?id=<?=$ticket->getId()?>" method="post">
    ...
    <tr><td align="left" colspan=2 class="msg">
         <?php translate('TEXT_UPDATE_TICKET'); ?> &nbsp;&nbsp;(<a href="tickets.php?id=<?=$ticket->getId()?>" style="color:black;"><?php translate('TEXT_VIEW_TICKET');?></a>)<br></td></tr>
    <tr>
    ...
    <?
      ...
      <tr>
        <td align="left"><?php translate('LABEL_PRIORITY')?>:</td>
        <td>
            <select name="pri">
              <?
                while($row=db_fetch_array($priorities)){ ?>
                    <option value="<?=$row['priority_id']?>" <?=$info['pri']==$row['priority_id']?'selected':''?> ><?php print eval('?>' .$row['priority_desc']. '<?php ');?></option>
              <?}?>
            </select>
        </td>
       </tr>
