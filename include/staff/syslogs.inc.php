...
$pagelimit=$_GET['limit']?$_GET['limit']:5;
...
?>
<div class="msg"><?php print translate("LABEL_SYSTEM_LOGS");?></div>
<div id='filter' >
 <form action="admin.php?t=syslog" method="get">
    <input type="hidden" name="t" value="syslog" />
    <div style="padding-left:15px;">
        <?php print translate("LABEL_DATE_SPAN");?>:
        &nbsp;<?php print translate("LABEL_FROM");?>&nbsp;<input id="sd" size=15 name="startDate" value="<?=Format::htmlchars($_REQUEST['startDate'])?>" 
            ...
            &nbsp;&nbsp;
            &nbsp;<?php print translate("LABEL_TYPE");?>:
            <select name='type'>
                <option value="" selected><?php print translate("LABEL_ALL");?></option>
                <option value="Error" <?=($type=='Error')?'selected="selected"':''?>><?php print translate("TEXT_ERRORS");?></option>
                <option value="Warning" <?=($type=='Warning')?'selected="selected"':''?>><?php print translate("TEXT_WARNINGS");?></option>
		<option value="Debug" <?=($type=='Debug')?'selected="selected"':''?>><?php print translate("TEXT_DEBUG");?></option>
            </select>
            &nbsp;&nbsp;
            &nbsp;<?php print translate("TEXT_LIMIT");?>:
            <select name='limit'>
            <?php 
            for ($x = 5; $x <= 50; $x += 5)
            {
            ...
            &nbsp;&nbsp;
            <input type="submit" class="button" Value='<?php print translate("LABEL_GO");?>' />
    </div>
 </form>
</div>
<div style="margin-bottom:20px; padding-top:0px;">
 <table width="100%" border="0" cellspacing=1 cellpadding=2>
<form action="admin.php?t=syslog" method="POST" name='syslog' onSubmit="return checkbox_checker(this,1,0);">
    <input type='hidden' name='t' value='syslog'>
    <input type="hidden" name="do" value="mass_process" >
    <input type="hidden" name="status" value="<?=$statusss?>" >
    <tr><td>
    <table width="100%" border="0" cellspacing=0 cellpadding=2 class="logs" align="center">&nbsp;<b><?=$showing?>&nbsp;&nbsp;&nbsp;<?=$results_type?></b>
     <tr><th>&nbsp;</th><th><?=$title?></th></tr>
        ...
        if($result && ($num=db_num_rows($result))):
		 ...
		 <tr class="<?=$class?> " id="<?=$row['log_id']?>">
                <td width=7px>
                 <input type="checkbox" name="ids[]" value="<?=$row['log_id']?>" <?=$sel?'checked':''?>  
    ...
    <tr>
        <td style="padding-left:20px">
            ...
            <a href="#" onclick="return select_all(document.forms['syslog'],true)"><?php print translate("LABEL_ALL");?></a>&nbsp;
            <a href="#" onclick="return reset_all(document.forms['syslog'])"><?php print translate("LABEL_NONE");?></a>&nbsp;
            <a href="#" onclick="return toogle_all(document.forms['syslog'],true)"><?php print translate("LABEL_TOGGLE");?></a>&nbsp;
        </td>
    </tr>
    <tr>
        <td align="center">
            <input class="button" type="submit" name="delete" value='<?php print translate("TEXT_DELETE_SELECTED_LOG");?>' 
                onClick=' return confirm("Are you sure you want to DELETE selected log record?");'>
        </td>
    </tr>
    <?
    endif;
