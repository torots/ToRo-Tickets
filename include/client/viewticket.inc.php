...
if(strcasecmp($thisclient->getEmail(),$ticket->getEmail())) die(translate("TEXT_ACCESS_DENIED"));
...
<table width="100%" cellpadding="1" cellspacing="0" border="0">
    <tr><td colspan=2 width=100% class="msg"><?= translate('TEXT_TICKET_NUMBER')?> <?=$ticket->getExtId()?> 
    ...
    <tr>
       <td width=50%>	
        <table align="center" class="infotable" cellspacing="1" cellpadding="3" width="100%" border=0>
	        <tr>
				<th width="100" ><?= translate('LABEL_TICKET_NUMBER')?>:</th>
				<td><?=$ticket->getStatus()?></td>
			</tr>
            <tr>
                <th><?= translate('LABEL_DEPARTMENT')?>:</th>
                <td><?=Format::htmlchars($dept->getName())?></td>
            </tr>
			<tr>
                <th><?= translate('LABEL_CREATE_DATE')?>:</th>
	   ...
	   <td width=50% valign="top">
        <table align="center" class="infotable" cellspacing="1" cellpadding="3" width="100%" border=0>
            <tr>
                <th width="100"><?= translate('LABEL_NAME')?>:</th>
                <td><?=Format::htmlchars($ticket->getName())?></td>
            </tr>
            <tr>
                <th width="100"><?= translate('LABEL_EMAIL')?>:</th>
...
<div class="msg"><?= translate('LABEL_SUBJECT')?>: <?=Format::htmlchars($ticket->getSubject())?></div>
<div>
    <?if($errors['err']) {?>
...
<br>
<div align="left">
    <span class="Icon thread"><?= translate('TEXT_TICKET_THREAD')?></span>
    <div id="ticketthread">
        <?
	    //get messages
...
<div>
    ...
    <div id="reply" style="padding:10px 0 20px 40px;">
        <?if($ticket->isClosed()) {?>
        <div class="msg"><?= translate('TEXT_TICKET_WILL_REOPEN_NEW_POST')?></div>
        <?}?>
        <form action="view.php?id=<?=$id?>#reply" name="reply" method="post" enctype="multipart/form-data">
            ...
            <div align="left">
                <?= translate('TEXT_ENTER_MESSAGE')?> <font class="error">*&nbsp;<?=$errors['message']?></font><br/>
                <textarea name="message" id="message" cols="60" rows="7" wrap="soft"><?=$info['message']?></textarea>
            </div>
            <? if($cfg->allowOnlineAttachments()) {?>
            <div align="left">
                <?= translate('TEXT_ATTACH_FILE')?><br><input type="file" name="attachment" id="attachment" size=30px value="<?=$info['attachment']?>" /> 
                    <font class="error">&nbsp;<?=$errors['attachment']?></font>
            </div>
            <?}?>
            <div align="left"  style="padding:10px 0 10px 0;">
                <input class="button" type='submit' value='<?= translate('LABEL_POST_REPLY')?>' />
                <input class="button" type='reset' value='<?= translate('LABEL_RESET')?>' />
                <input class="button" type='button' value='<?= translate('LABEL_CANCEL')?>' onClick='window.location.href="view.php"' />
            </div>
        </form>
    </div>
</div>
