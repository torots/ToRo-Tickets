...
<div style="margin: 10px 0 60px 0;">
 <table width="100%" border="0" cellspacing=0 cellpadding=0 align="center">
    <tr>
        ...
        <td nowrap >
            <a href="view.php?status=open"><img src="images/view_open_btn.gif" alt="<?= translate('TEXT_VIEW_OPEN')?>" border=0></a>            
            <a href="view.php?status=closed"><img src="images/view_closed_btn.gif" alt="<?= translate('TEXT_VIEW_CLOSED')?>" border=0></a>            
            <a href=""><img src="<?php tei('IMAGE_STAFF_REFRESH')?>" alt="<?= translate('TEXT_REFRESH')?>" border=0></a>
        </td>
    </tr>
 </table>
 <table width="100%" border="0" cellspacing=0 cellpadding=2>
    <tr><td>
     <table border="0" cellspacing=0 cellpadding=2 class="tgrid" align="center">
        <tr>
	        <th width="70" nowrap>
                <a href="view.php?sort=ID&order=<?=$negorder?><?=$qstr?>" title="<?= translate('TEXT_SORT_BY_TICKET_ID')?> <?=$negorder?>"><?= translate('LABEL_TICKET_NUMBER')?></a></th>
	        <th width="100">
                <a href="view.php?sort=date&order=<?=$negorder?><?=$qstr?>" title="<?= translate('TEXT_SORT_BY_DATE')?> <?=$negorder?>"><?= translate('LABEL_CREATE_DATE')?></a></th>
            <th width="60"><?= translate('LABEL_TICKET_STATUS')?></th>
            <th width="240"><?= translate('LABEL_SUBJECT')?></th>
            <th width="150">
                <a href="view.php?sort=dept&order=<?=$negorder?><?=$qstr?>" title="<?= translate('TEXT_SORT_BY_DEPARTMENT')?> <?=$negorder?>"><?= translate('LABEL_DEPARTMENT')?></a></th>
            <th width="150"><?= translate('LABEL_EMAIL')?></th>
        </tr>
        <?
        $class = "row1";
        $total=0;
        if($tickets_res && ($num=db_num_rows($tickets_res))):
            ...
            <tr class="<?=$class?>"><td colspan=7><b><?= translate('TEXT_NO_TICKETS_FOUND')?></b></td></tr>
        <?
        endif; ?>
     </table>
    </td></tr>
