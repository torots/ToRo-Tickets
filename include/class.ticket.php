...
/*********************************************************************
    ...
    $Id: class.ticket.php,v 1.6 2011/10/26 20:20:01 root Exp $
**********************************************************************/
...
class Ticket{
    ...
    $thisStaff=$thisuser->getId();

        db_query('UPDATE '.TICKET_TABLE.' SET isanswered=1,lastresponse=NOW(), updated=NOW(),staff_id='.$thisStaff.' WHERE ticket_id='.db_input($this->getId()));
    }
            ...
            if ( db_query($sql5) && $msgid=db_insert_id() ); {  // && $msgid=db_insert_id($res5)
              ...
              if($rest && db_num_rows($rest))
            	...
            	$sqlm = 'SELECT m.msg_id FROM '.TICKET_MESSAGE_TABLE.' m, '.TICKET_TABLE.' t 
		      	    WHERE t.ticketID='.$ticketID.' AND m.ticket_id=t.ticket_id ORDER BY msg_id ASC LIMIT 1'; 
              $resm=db_query($sqlm);
   ...
   /*============== Functions below do not require an instance of the class to be used. To call it use Ticket::function(params); ==================*/
    ...
    function create($var,&$errors,$origin,$autorespond=true,$alertstaff=true) {
       ...
       /* Coders never code so fully and joyfully as when they do it for free  - Peter Rotich */
        ...
        $id=0;
        $fields=array();
        $fields['name']     = array('type'=>'string',   'required'=>1, 'error'=>translate('ERROR_NAME_REQUIRED'));
        $fields['email']    = array('type'=>'email',    'required'=>1, 'error'=>'Valid email required');
        $fields['subject']  = array('type'=>'string',   'required'=>1, 'error'=>'Subject required');
        $fields['message']  = array('type'=>'text',     'required'=>1, 'error'=>translate('ERROR_MESSAGE_REQUIRED'));
