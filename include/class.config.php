...
/*********************************************************************
 ...
 $Id: class.config.php,v 1.5 2011/10/26 18:48:31 root Exp $
 **********************************************************************/
...
class Config {
	...
	function enableToolTips() {
		 return $this->config['tool_tips']?true:false;
	}
	...
	function assignOnResponse() {
		return $this->config['assign_on_response']?true:false;
	}
	...
	function updatePref($var,&$errors) {
		...
		$f['client_motd']=array('type'=>'string',   'required'=>0, 'error'=>'Helpdesk MotD required');
		$f['staff_motd']=array('type'=>'string',   'required'=>0, 'error'=>'Helpdesk MotD required');
		...
		if($errors) return false; //No go!
                ...
                $motd_lastupdateddate = date('Y-m-d H:i:s');
	    ...
	    ',tool_tips='.db_input(isset($var['enable_tooltips'])?1:0).
            ...
            ',assign_on_response='.db_input(isset($var['assign_on_response'])?1:0).
            ...
            ',client_motd='.db_input($var['client_motd']).
            ',client_motd_lastupdated='.db_input($motd_lastupdateddate).
            ',staff_motd='.db_input($var['staff_motd']).
            ',staff_motd_lastupdated='.db_input($motd_lastupdateddate).
