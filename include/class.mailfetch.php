...
class MailFetcher {
    ...
    function createTicket($mid,$emailid=0){
        ...
        $var['name']=iconv_mime_decode($mailinfo['from']['name'],0,"UTF-8");
        $var['email']=$mailinfo['from']['email'];
        $var['subject']=$mailinfo['subject']?iconv_mime_decode($mailinfo['subject'],0,"UTF-8"):'[No Subject]';
	...
	Sys::log(LOG_DEBUG,'Count Messages','Number of messages [$nummsgs]');
	$msgs=$errors=0;
    ...
    function fetchMail(){
    Sys::log(LOG_DEBUG,'function fetchMail','fetchMail Activated');
        global $cfg;
        ...
        $sql=' SELECT email_id,mail_host,mail_port,mail_protocol,mail_encryption,mail_delete,mail_errors,userid,userpass FROM '.EMAIL_TABLE.
             ...
             ' AND (mail_lastfetch=0 OR TIME_TO_SEC(TIMEDIFF(NOW(),mail_lastfetch))>mail_fetchfreq*60) ';




        echo $sql;
	Sys::log(LOG_DEBUG,'SQL',"$sql");
        ...
        while($row=db_fetch_array($accounts)) {
            ...
            if($fetcher->connect()){   
	    Sys::log(LOG_DEBUG,'Fetcher Connected','Yep');
