...
/*********************************************************************
    ...
    $Id: class.staff.php,v 1.11 2011/10/18 16:26:38 root Exp $
**********************************************************************/
class Staff {
    ...
    // toroTS authentication
     ...
     case '2':
      ...
      $ds=ldap_connect($auth_data,$auth_port) or die("Couldn't connect to AD!");
      ...
      if ($ds) {
       ...
       if (!@ldap_bind( $ds, $auth_dn."\\".$this->username, $password) ) { 
        // auth failed if failover = yes try toroTS auth
        if($auth_fail=='1') {
         return (strlen($this->passwd) && strcmp($this->passwd, MD5($password))==0)?(TRUE):(FALSE); 
        }
        else {
