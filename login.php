...
/*********************************************************************
    ...
    $Id: login.php,v 1.1 2011/08/20 13:58:31 ntozier Exp $
**********************************************************************/
...
//We are ready baby
$loginmsg=translate('TEXT_AUTHENTICATION_REQUIRED');
if($_POST && (!empty($_POST['lemail']) && !empty($_POST['lticket']))):
    ...
    $loginmsg=translate('TEXT_INVALID_LOGIN');;
    if($_SESSION['_client']['laststrike']) {
        if((time()-$_SESSION['_client']['laststrike'])<$cfg->getClientLoginTimeout()) {
            $loginmsg=translate('ALERT_EXCESSIVE_LOGIN_ATTEMPTS_BY_CLIENT');
            $errors['err']=translate('ERROR_YOU_HAVE_REACHED_MAXIMUM_FAILED_LOGIN');
    ...
    if(!$errors && $_SESSION['_client']['strikes']>$cfg->getClientMaxLogins()) {
        $loginmsg=translate('TEXT_ACCESS_DENIED');
        $errors['err']=translate('TEXT_FORGOT_YOUR_LOGIN_INFO');
        ...
        //$alert=translate('ALERT_EXCESSIVE_LOGIN_ATTEMPTS_BY_CLIENT',$param);
