...
/*********************************************************************
    ...
    $Id: main.inc.php,v 1.2 2011/12/19 18:19:49 root Exp $
**********************************************************************/    
    ...
    #Current version..
    define('THIS_VERSION','1.0 BETA'); //Changes from version to version.
    ...
    elseif(file_exists(INCLUDE_DIR.'toro-config.php')) //NEW cfg file v 1.6 stable
        $configfile=INCLUDE_DIR.'toro-config.php';
   ...
   //Path separator
    ...
    require(INCLUDE_DIR.'class.language.php'); // New language application
    ...
    define('LANGUAGE_TABLE',TABLE_PREFIX.'languages');
    define('CONFIG_TABLE',TABLE_PREFIX.'config');
    define('SYSLOG_TABLE',TABLE_PREFIX.'syslog');
    define('COLUMNS_TABLE',TABLE_PREFIX.'columns');
    define('THEME_TABLE',TABLE_PREFIX.'theme');
    ...
    if (!db_connect(DBHOST,DBUSER,DBPASS) || !db_select_database(DBNAME)) {
      ...
      // once we change all short tags to long tags this portion can be removed
        $ferror='Short open tag disabled! - osTicket required it turned ON.';
    }

    if($ferror){ //Fatal error
        Sys::alertAdmin('toroTS Fatal Error',$ferror); //try alerting admin.
