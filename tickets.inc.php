<?php
if(!defined('OSTSCPINC') || !@$thisuser->isStaff()) die(translate("TEXT_ACCESS_DENIED"));
...
<div id='basic' style="display:<?=$basic_display?'block':'none'?>">
    ...
    <table>
        <tr>
            <td><?php  translate(LABEL_QUERY);?>: </td>
            <td><input type="text" id="query" name="query" size=30 value="<?=Format::htmlchars($_REQUEST['query'])?>"></td>
            <td><input type="submit" name="basic_search" class="button" value="<?php echo translate(LABEL_SEARCH); ?>">
             &nbsp;[ <a href="#" onClick="showHide('basic','advance'); return false;"><?php echo translate(LABEL_ADVANCED);?></a> ] </td>
        </tr>
    </table>
    </form>
</div>
<div id='advance' style="display:<?=$basic_display?'none':'block'?>">
 ...
 <input type="hidden" name="a" value="search">
  <table>
    <tr>
        <td><?php translate(LABEL_QUERY);?>: </td><td><input type="text" id="query" name="query" value="<?=Format::htmlchars($_REQUEST['query'])?>"></td>
        <td><?php translate(LABEL_DEPT);?>:</td>
        <td><select name="dept"><option value=0><?php echo translatet(TEXT_ALL_DEPARTMENTS)?></option>
            <?
        ...
        <td><?php translate(LABEL_STATUS_IS)?>:</td><td>
    
        <select name="status">
            <option value='any' selected ><?php echo translate(TEXT_ANY_STATUS)?></option>
            <option value="open" <?=!strcasecmp($_REQUEST['status'],'Open')?'selected':''?>><?php echo translate('TEXT_OPEN');?></option>
            <option value="overdue" <?=!strcasecmp($_REQUEST['status'],'overdue')?'selected':''?>><?php echo translate('TEXT_OVERDUE');?></option>
            <option value="closed" <?=!strcasecmp($_REQUEST['status'],'Closed')?'selected':''?>><?php echo translate('TEXT_CLOSED');?></option>
        </select>
        </td>
     </tr>
    </table>
    <div>
        <?php translate('LABEL_DATE_SPAN');?>:
        &nbsp;<?php translate('LABEL_FROM');?>&nbsp;<input id="sd" name="startDate" value="<?=Format::htmlchars($_REQUEST['startDate'])?>" 
                onclick="event.cancelBubble=true;calendar(this);" autocomplete=OFF>
            <a href="#" onclick="event.cancelBubble=true;calendar(getObj('sd'));"><img src='images/cal.png'border=0 alt=""></a>
            &nbsp;&nbsp; <?php translate('LABEL_TO');?> &nbsp;&nbsp;
    ...
    <table>
    <tr>
       <td><?php translate('LABEL_TYPE');?>:</td>
       <td>       
        <select name="stype">
       ...
       <td><?php echo translate('LABEL_SORT_BY');?>::</td><td>
        <?php 
         $sort=$_GET['sort']?$_GET['sort']:'date';
        ?>
        <select name="sort">
    	    <option value="ID" <?= $sort== 'ID' ?'selected':''?>><?php echo translate(LABEL_TICKET_NUMBER)?></option>
            <option value="pri" <?= $sort == 'pri' ?'selected':''?>><?php echo translate(LABEL_PRIORITY)?></option>
            <option value="date" <?= $sort == 'date' ?'selected':''?>><?php echo translate(LABEL_DATE)?></option>
            <option value="dept" <?= $sort == 'dept' ?'selected':''?>><?php echo translate(LABEL_DEPT)?></option>
        </select>
        <select name="order">
            <option value="DESC"<?= $_REQUEST['order'] == 'DESC' ?'selected':''?>><?php echo translate(LABEL_DESCENDING)?></option>
            <option value="ASC"<?= $_REQUEST['order'] == 'ASC'?'selected':''?>><?php echo translate(LABEL_ASCENDING)?></option>
        </select>
       </td>
        <td><?php echo translate(LABEL_RESULTS_PER_PAGE)?>:</td><td>
        <select name="limit">
        <?
     ...
     <td>
     <input type="submit" name="advance_search" class="button" value="<?php echo translate(LABEL_SEARCH)?>">
       &nbsp;[ <a href="#" onClick="showHide('advance','basic'); return false;" ><?php echo translate(LABEL_BASIC)?></a> ]
    </td>
  </tr>
 </table>
 </form>
 ...
 <table width="100%" border="0" cellspacing=1 cellpadding=2>
	...
	?>
		...
		//print $tdList;
            ...
            <tr class="<?=$class?>"><td colspan=8><b><?php translate('LABEL_QUERY_RETURNED_ZERO_RESULT')?></b></td></tr>
        <?
        endif; ?>
       </table>
    </td></tr>
    ...
    ?>
        <tr><td style="padding-left:20px">
            ...
            <?php translate('LABEL_SELECT')?>:
                <a href="#" onclick="return select_all(document.forms['tickets'],true)"><?php translate('LABEL_ALL')?></a>&nbsp;
                <a href="#" onclick="return reset_all(document.forms['tickets'])"><?php translate('LABEL_NONE')?></a>&nbsp;
                <a href="#" onclick="return toogle_all(document.forms['tickets'],true)"><?php translate('LABEL_TOgGLE')?></a>&nbsp;
            <?}?>
            <?php translate('LABEL_PAGE')?>:<?=$pageNav->getPageLinks()?>
        </td></tr>
        ...
        <tr><td align="center"> <br>
            ...
            switch (strtolower($status)) {
                case 'closed': ?>
                    <input class="button" type="submit" name="reopen" value="<?php translate('LABEL_REOPEN')?>"
                ...
                case 'assigned':
                    ?>
                    <input class="button" type="submit" name="overdue" value="<?php translate('LABEL_OVERDUE')?>"
                        onClick=' return confirm("Are you sure you want to mark selected tickets overdue/stale?");'>
                    <input class="button" type="submit" name="close" value="<?php translate('LABEL_CLOSE')?>"
	        ...
	        <input class="button" type="submit" name="banDelete" value='<?php print translate("LABEL_BAN_AND_DELETE");?>'
                ...
                <input class="button" type="submit" name="delete" value="<?php translate('LABEL_DELETE')?>" 
