...
/*********************************************************************
    ...
    Peter Rotich <peter@ToRo.com>
    Copyright (c)  2006-2010 ToRo
    http://www.ToRo.com
...
define('VERSION','1.0 BETA'); //Current version number
define('VERSION_VERBOSE','1.0 Beta'); //What the user sees during installation process.
define('CONFIGFILE','../include/toro-config.php'); //ToRo config file full path.
...
define('SCHEMAFILE','./inc/torots-v1.sql'); //ToRo SQL schema.
define('URL',rtrim('http'.(($_SERVER['HTTPS']=='on')?'s':'').'://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']),'setup'));

$install='<strong>Need help?</strong> <a href="http://www.torots.com/support/" target="_blank">Professional Installation Available</a>';
$support='<strong>Get a peace of mind</strong> <a href="http://www.torots.com/support/" target="_blank">Commercial Support Available</a>';
...
if(file_exists('../ostconfig.php') || file_exists('../include/settings.php')) { //old installation? try upgrading it buddy.
    ...
    $errors['err']='Short open tag disabled! - ToRo requires it turned on.';
    ...
    if(preg_match("/define\('OSTINSTALLED',TRUE\)\;/i",$configfile) || !strpos($configfile,'%CONFIG-DBHOST')){
        ...
        if($_POST['prefix'] && substr($_POST['prefix'], -1)!='_')
            $errors['prefix']='Bad prefix. Must have underscore (_) at the end. e.g \'toro_\'';
       
        //Make sure admin username is not very predictable.
        if(!$errors['username'] && in_array(strtolower($_POST['username']),array('admin','admins','username','ToRo')))
        ...
        if(!$errors && (db_version()<'4.1.1'))
            $errors['mysql']='ToRo requires MySQL 4.1.1 or better! Please upgrade';
            ...
            if(!$errors) {
                ...
                if(ftruncate($fp,0) && fwrite($fp,$configfile)){
                    ...
                    $sql='INSERT INTO '.PREFIX.'email SET created=NOW(),updated=NOW(),priority_id=1,dept_id=1'.
                         ',name='.db_input('ToRo Alerts').',email='.db_input('alerts@'.$domain);
                    mysql_query($sql);
                    //3 - noreply email
                    $sql='INSERT INTO '.PREFIX.'email SET created=NOW(),updated=NOW(),priority_id=1,dept_id=1'.
                    ...
                    $sql='INSERT INTO '.PREFIX.'ticket SET created=NOW(),ticketID='.db_input(Misc::randNumber(6)).
                        ",priority_id=2,dept_id=1,email='support@ToRo.com',name='ToRo Support' ".
                        ",subject='ToRo Installed!',helptopic='Commercial support',status='open',source='Web'";
                    if(db_query($sql) && ($id=db_insert_id())){
                        db_query('INSERT INTO '.PREFIX."ticket_message VALUES (1,$id,NULL,".db_input(TORO_INSTALLED).",NULL,'Web','',NOW(),NULL)");
                    }
                    //Log a message.
                    $sql='INSERT INTO '.PREFIX.'syslog SET created=NOW(),updated=NOW() '.
                         ',title="ToRo installed!",log_type="Debug" '.
                         ',log='.db_input("Congratulations ToRo basic installation completed!\n\nThank you for choosing ToRo!").
                         ',ip_address='.db_input($_SERVER['REMOTE_ADDR']);
                    mysql_query($sql);
                    $msg='Congratulations ToRo basic installation completed!';
                    $inc='done.inc.php';
                }else{
...
$title=sprintf('ToRo version %s - Basic installation',VERSION_VERBOSE);
?>
<html>
<head>
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
<title>ToRo Installer</title>
<link rel="stylesheet" href="style.css" media="screen">
</head>
<body>
<div id="container">
    <div id="header">
        <a id="logo" href="#" title="ToRo"><img src="images/toroLogo.png" width="188" height="72" alt="ToRo Installer"></a>
        <p id="info"><?=$info?></p>
    </div>
    <div id="nav">
        <ul id="sub_nav">
    ...
    <div id="footer">Copyright &copy; <?=date('Y')?>&nbsp;ToRo &nbsp;All Rights Reserved.</div>
</div>
</body>
</html>
