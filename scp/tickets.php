...
a    tickets.php
    ...
    $Id: tickets.php,v 1.5 2012/01/04 01:47:46 ntozier Exp $
**********************************************************************/

require_once('staff.inc.php');
require_once(INCLUDE_DIR.'class.language.php');
    ...
    if($ticket && $ticket->getId()) {
        ...
        case 'reply':
            ...
            if(!$errors && ($respId=$ticket->postResponse($_POST['msg_id'],$_POST['response'],$_POST['signature'],$_FILES['attachment']))){
                $msg=translate('TEXT_RESPONSE_POSTED_SUCCESSFULLY');
                //Set status if any.
	...
	$ticket->logActivity('Ticket status changed to '.($ticket->isOpen()?'Open':'Closed'),$note);
        ...
        case 'transfer':
            ...
            if(!$errors && $ticket->transfer($_POST['dept_id'])){
                ...
                $msg=translate('TEXT_TICKET_TRANSFERED_SUCESSFULLY_TO_DEPT',$ticket->getDeptName());
	...
	case 'mergeticket':
        ...
        case 'assign':
            ...
            if(!$errors && $ticket->assignStaff($_POST['staffId'],$_POST['assign_message'])){
                ...
                $msg=translate('TEXT_TICKET_ASSIGNED_TO_STAFF',($staff?$staff->getName():translate('LABEL_STAFF')));
	...
	case 'postnote':
        ...
        case 'update':
            ...
            elseif($ticket->update($_POST,$errors)){
                $msg=translate('TEXT_TICKET_UPDATED_SUCCESSFULLY');
            ...
            switch(strtolower($_POST['do'])):
                case 'change_priority':
                    ...
                    if(!$errors){
                        if($ticket->setPriority($_POST['ticket_priority'])){
                            $msg=translate('TEXT_PRIORITY_CHANGED_SUCCESSFULLY');
                            $ticket->reload();
                ...
                case 'close':
                    if(!$thisuser->isadmin() && !$thisuser->canCloseTickets()){
                        ...
                        if($ticket->close()){
                            $msg=translate('TEXT_TICKET_NUM_STATUS_SET_CLOSED',$ticket->getExtId());
                ...
                case 'reopen':
                    ...
                    if(!$thisuser->isadmin() && !$thisuser->canCloseTickets()){
                        ...
                        if($ticket->reopen()){
                            $msg=translate('TEXT_TICKET_STATUS_SET_TO_OPEN');
                ...
                case 'overdue':
                    ...
                    if(!$thisuser->isadmin() && !$thisuser->isManager()){
                        ...
                        if($ticket->markOverdue()){
                            $msg=translate('TEXT_TICKET_FLAGGED_AS_OVERDUE');
                            $note=$msg;
                            if($_POST['ticket_priority']) {
                                $ticket->setPriority($_POST['ticket_priority']);
                                $ticket->reload();
                ...
                case 'banemail':
                    if(!$thisuser->isadmin() && !$thisuser->canManageBanList()){
                        ...
                        $msg=translate('TEXT_EMAIL_ADDED_TO_BANLIST',$ticket->getEmail());;
                        if($ticket->isOpen() && $ticket->close()) {
                            $msg.=translate('TEXT_AND_TICKET_STATUS_SET_TO_CLOSED');
                ...
                case 'unbanemail':
                    if(!$thisuser->isadmin() && !$thisuser->canManageBanList()){
                        ...
                        $msg=translate('TEXT_EMAIL_REMOVED_FROM_BANLIST');
                    }else{
                ...
                case 'delete': // Dude what are you trying to hide? bad customer support??
                    if(!$thisuser->isadmin() && !$thisuser->canDeleteTickets()){
                        ...
                        if($ticket->delete()){
                            ...
                            $msg=translate('TEXT_TICKET_DELETED_FOREVER');
	...
	if(!$errors) {
            ...
            case 'open':
                ...
                if(($ticket=Ticket::create_by_staff($_POST,$errors))) {
                    $ticket->reload();
                    $msg=translate('TEXT_TICKET_CREATED_SUCCESSFULLY');
...
if($cfg->showAnsweredTickets()) {
	$desc  = translate("TEXT_TOTAL_OPEN_TICKETS");
	$desc .= " (" .($stats['open']+$stats['answered']). ")";
	$title = translate("LABEL_OPEN_TICKET");
...
//    if($stats['open'])
    	$desc  = translate("TEXT_TOTAL_OPEN_TICKETS");
	$desc .= " (" .$stats['open']. ")";
    	$title  = translate("TEXT_OPEN_TICKETS");
        $nav->addSubMenu(array('desc'=>$desc,'title'=>$title, 'href'=>'tickets.php', 'iconclass'=>'Ticket'));
    if($stats['answered']) {
	$desc = translate("TEXT_TOTAL_ANSWERED_TICKETS");
	$desc .= " (" .$stats['answered']. ")";
    	$title = translate("TEXT_ANSWERED_TICKETS");
        $nav->addSubMenu(array('desc'=>$desc,
...
if($stats['assigned']) {
    ...
    $desc  = translate('TEXT_TOTAL_MY_TICKETS');
    $desc .= " (" .$stats['answered']. ")"; 
    $title = translate('LABEL_ASSIGNED_TICKETS');
...
$desc=translate("PENDING_TICKETS");
$desc.= " (" .$stats['pending']. ")";
$nav->addSubMenu(array('desc'=>$desc,'title'=>'Pending Tickets', 'href'=>'tickets.php?status=pending', 'iconclass'=>'closedTickets'));

$desc = translate('LABEL_CLOSED_TICKETS');
...
if($thisuser->canCreateTickets()) {
	$desc = translate('LABEL_NEW_TICKET');
