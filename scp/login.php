...
/*********************************************************************
    ...
    $Id: login.php,v 1.1 2011/10/24 21:13:38 root Exp $
**********************************************************************/
...
$msg=$msg?$msg:translate('TEXT_AUTHENTICATION_REQUIRED');
$labelUsername=translate('LABEL_USERNAME');
$labelPassword=translate('LABEL_PASSWORD');
...
if($_POST && (!empty($_POST['username']) && !empty($_POST['passwd']))){
    ...
    $msg=translate('TEXT_INVALID_LOGIN');
    if($_SESSION['_staff']['laststrike']) {
       $fatorLastStrike = (time()-$_SESSION['_staff']['laststrike']);
	   if($fatorLastStrike<$cfg->getStaffLoginTimeout()) {
           $msg=translate('TEXT_EXCESSIVE_FAILED_LOGIN_ATTEMPTS');
            $errors['err']=translate('TEXT_YOUVE_REACHED_MAXIMUM_FAILED_LOGIN_ATTEMPTS_ALLOWED',$fatorLastStrike);
    ...
    if(!$errors && ($user=new StaffSession($_POST['username'])) && $user->getId() && $user->check_passwd($_POST['passwd'])){
        ...
        define("TOROPVTLTD",TRUE); //Make includes happy        
        // do tasks prior to redirecting
        include_once(INCLUDE_DIR.'staff/prelogin.inc.php');
        exit;
    }
    //If we get to this point we know the login failed.
    $_SESSION['_staff']['strikes']+=1;
    if(!$errors && $_SESSION['_staff']['strikes']>$cfg->getStaffMaxLogins()) {
        $msg=translate("TEXT_ACCESS_DENIED");
