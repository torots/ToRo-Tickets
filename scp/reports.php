...
$qselect = "SELECT name,email,
            COUNT(DISTINCT(toro_ticket.ticket_id)) AS number,
            COUNT(DISTINCT(CASE WHEN toro_ticket.status='open' THEN toro_ticket.ticket_id END)) as opened,
            COUNT(DISTINCT(CASE WHEN toro_ticket.status='closed' THEN toro_ticket.ticket_id END)) as closed
            FROM toro_ticket";
}
elseif($_POST['type'] == 'repliesPerStaff'){
$qselect = "SELECT
            toro_staff.lastname,toro_staff.firstname,toro_ticket_response.response_id,
	        toro_ticket_response.staff_id,toro_ticket_response.created,
            COUNT(DISTINCT(toro_ticket_response.response_id)) as responses FROM toro_ticket_response
            LEFT JOIN toro_ticket ON toro_ticket.ticket_id=toro_ticket_response.ticket_id
            LEFT JOIN toro_staff ON toro_staff.staff_id=toro_ticket_response.staff_id ";
}
elseif($_POST['type'] == 'tixPerTopic'){
$qselect = "SELECT                                         
            toro_ticket.helptopic,
            COUNT(DISTINCT(toro_ticket.ticket_id)) AS number FROM toro_ticket";
}else{
$qselect = "SELECT
            ROUND(AVG(TIMESTAMPDIFF(HOUR, toro_ticket.created, toro_ticket.closed)),2) AS hoursAVG,
            ROUND(AVG(TIMESTAMPDIFF(DAY, toro_ticket.created, toro_ticket.closed)),2) AS daysAVG,
            toro_ticket.dept_id,toro_ticket.staff_id,toro_staff.staff_id,toro_staff.firstname,toro_staff.lastname,
            toro_ticket.created,toro_ticket.updated,toro_ticket.closed,toro_department.dept_name,
            COUNT(DISTINCT(toro_ticket.ticket_id)) AS number,
            COUNT(DISTINCT(CASE WHEN toro_ticket.status='open' THEN toro_ticket.ticket_id END)) as opened,
            COUNT(DISTINCT(CASE WHEN toro_ticket.status='closed' THEN toro_ticket.ticket_id END)) as closed   
            FROM toro_ticket
            LEFT JOIN toro_ticket_response ON toro_ticket_response.ticket_id=toro_ticket.ticket_id
            LEFT JOIN toro_staff ON toro_ticket.staff_id=toro_staff.staff_id
            LEFT JOIN toro_department ON toro_ticket.dept_id=toro_department.dept_id";
}
...
if($_POST['dateRange'] == 'timePeriod'){
  ...
  $qwhere = "WHERE toro_ticket.created>=CURDATE() ";
  $report_range = "Today";
  }

  // Yesterday
  if($_POST['range'] == 'yesterday'){     
  $qwhere = "WHERE toro_ticket.created>=DATE_ADD(CURDATE(), INTERVAL -1 DAY) AND toro_ticket.created<CURDATE()";
  $report_range = "Yesterday";
  }

  // This month
  if($_POST['range'] == 'thisMonth'){
  $qwhere = "WHERE YEAR(toro_ticket.created) = YEAR(CURDATE()) AND MONTH(toro_ticket.created) >= MONTH(CURDATE())";
  $report_range = "This Month";
  }

  // Last month
  if($_POST['range'] == 'lastMonth'){
  $qwhere = "WHERE YEAR(toro_ticket.created) = YEAR(CURDATE()) AND MONTH(toro_ticket.created) >= MONTH(DATE_ADD(CURDATE(),INTERVAL -1 MONTH)) AND MONTH(toro_ticket.created) < MONTH(CURDATE())";
  $report_range = "Last Month";
  }

  // Last 30 days
  if($_POST['range'] == 'lastThirty'){
  $qwhere = "WHERE toro_ticket.created>=DATE_ADD(CURDATE(), INTERVAL -30 DAY) ";
  $report_range = "Last 30 Days";
  }

  // This week (Sun-Sat)
  if($_POST['range'] == 'thisWeek'){
  $qwhere = "WHERE toro_ticket.created>=DATE_ADD(CURDATE(), interval(1 - DAYOFWEEK(CURDATE()) ) DAY) AND toro_ticket.created<=DATE_ADD(CURDATE(), interval(7 - DAYOFWEEK(CURDATE()) ) DAY)";
  $report_range = "This Week (Sun-Sat)";
  }

  // Last week (Sun-Sat)
  if($_POST['range'] == 'lastWeek'){
  $qwhere = "WHERE toro_ticket.created>=DATE_ADD(DATE_ADD(CURDATE(), interval(1 - DAYOFWEEK(CURDATE()) ) DAY), INTERVAL - 1 WEEK) AND toro_ticket.created<=DATE_ADD(DATE_ADD(CURDATE(), interval(7 - DAYOFWEEK(CURDATE()) ) DAY), interval  - 1 week)";               
  $report_range = "Last Week (Sun-Sat)";
  }

  // This Business week (Mon-Fri)
  if($_POST['range'] == 'thisBusWeek'){
  $qwhere = "WHERE toro_ticket.created>=DATE_ADD(CURDATE(), interval(2 - DAYOFWEEK(CURDATE()) ) DAY) AND toro_ticket.created<=DATE_ADD(CURDATE(), interval(6 - DAYOFWEEK(CURDATE()) ) DAY)";
  $report_range = "This Week (Mon-Fri)";
  }

  // Last Business week (Mon-Fri)
  if($_POST['range'] == 'lastBusWeek'){
  $qwhere = "WHERE toro_ticket.created>=DATE_ADD(DATE_ADD(CURDATE(), interval(2 - DAYOFWEEK(CURDATE()) ) DAY), INTERVAL - 1 WEEK) AND toro_ticket.created<=DATE_ADD(DATE_ADD(CURDATE(), interval(6 - DAYOFWEEK(CURDATE()) ) DAY), interval - 1 week)";
  $report_range = "Last Business Week (Mon-Fri)";
  }

  // This year
  if($_POST['range'] == 'thisYear'){
  $qwhere = "WHERE YEAR(toro_ticket.created) = YEAR(CURDATE()) ";
  $report_range = "This Year";
  }

  // Last year
  if($_POST['range'] == 'lastYear'){
  $qwhere = "WHERE YEAR(toro_ticket.created) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 YEAR)) "; 
...
if($_POST['dateRange'] == 'timeRange'){
  ...
  $toDate = $_POST['toDate'];
  $qwhere = "WHERE toro_ticket.created>=\"$fromDate 00:00:00\" AND toro_ticket.created<=\"$toDate 23:59:59\" ";
...
$qgroup = "GROUP BY toro_department.dept_id ORDER BY number DESC";
$report_type = "Tickets Per Department";
}
elseif($_POST['type'] == 'tixPerStaff'){
$qgroup = "GROUP BY toro_staff.staff_id ORDER BY toro_staff.lastname ";
...
elseif($_POST['type'] == 'tixPerDay'){
$qgroup = "GROUP BY DATE_FORMAT(toro_ticket.created, '%d %M %Y') ORDER BY toro_ticket.created  ";
$report_type = "Tickets Per Day";
}
elseif($_POST['type'] == 'tixPerMonth'){
$qgroup = "GROUP BY DATE_FORMAT(toro_ticket.created, '%M %Y') ORDER BY toro_ticket.created";
$report_type = "Tickets Per Month";
}
elseif($_POST['type'] == 'tixPerTopic'){
$qgroup = "GROUP BY toro_ticket.helptopic ORDER BY toro_ticket.helptopic";
