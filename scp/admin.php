...
/*********************************************************************
    ...
    $Id: admin.php,v 1.8 2011/10/25 14:55:50 root Exp $
**********************************************************************/
...
if(defined('THIS_VERSION') && strcasecmp($cfg->getVersion(),THIS_VERSION)) {
    ...
    if(file_exists('../setup/')){
        $sysnotice=translate('TEXT_PLEASE_TAKE_A_MINUTE_TO_DELETE_INSTALL');
    }else{
...
if($_POST && $_REQUEST['t'] && !$errors):
    ...
    if($_POST['submit']){
         ...
         if(!$_POST['enable_failover']) { $failover = '0'; }
         else { $failover = '1'; }
         $sql = "UPDATE ".CONFIG_TABLE." SET authentication_type='" .$_POST['auth']. "',"
               ."authentication_server='".$_POST['ldapserver']. "',"
               ."authentication_ldapport='".$_POST['ldapport']. "',"
               ."authentication_ldapdn='".$_POST['ldapdn']. "',"
               ."authentication_ldapver='".$_POST['ldapver']. "',"
               ."authentication_failover='".$failover."';";
        ...
        case 'dashboard':
            //require_once(INCLUDE_DIR.'/reports/timecalc_currentyear.php');
            ...
            switch($do){
                case 'mass_process':
                    if(!$_POST['ids'] || !is_array($_POST['ids'])) {
                        ...
                        $count=count($_POST['ids']);
                        $ids=implode(',',$_POST['ids']);
                        $sql='SELECT count(log_id) FROM '.SYSLOG_TABLE.' WHERE log_id IN ('.$ids.')';
                        list($sys)=db_fetch_row(db_query($sql));
                        if($sys<0){
                            $errors['err']='One or more of the selected syslog has/have been removed by another admin.';    
                        }elseif($_POST['delete']){
                            $i=0;
                            foreach($_POST['ids'] as $k=>$v) {
                                if(Sys::deleteSysLog($v)) $i++;
                            }
                            if($i>0){
                                $msg="$i of $count selected syslog(s) deleted";
                            }else{
                                $errors['err']='Unable to delete selected syslog(s).';
                            }
                        }else{
                            $errors['err']='Unknown command';
                        }
    ...
    if($_POST['defaultLang']){
     $sql = "UPDATE ".CONFIG_TABLE." SET LANGUAGE_ID=" .$_POST['defaultLang'];
     $result = mysql_query($sql) or die (mysql_error());
    ...
    if($_POST['newLang']){
     $sql = "INSERT INTO ".LANGUAGE_TABLE." (language_name) VALUES ('".addslashes($_POST['newLang'])."')";    
     $result = mysql_query($sql)or die (mysql_error());
     $msg = "New language added successfully";
    }

    // Add field to languages

    if(($_POST['fieldName']) && ($_POST['fieldName']!='')){
     $sql = "ALTER TABLE  ".LANGUAGE_TABLE." ADD  `".$_POST['fieldName']."` TEXT NOT NULL";
     $result = mysql_query($sql)or die(mysql_error());
     $msg = $_POST['fieldName']. " Added successfully";
    ...
    if($_POST['updateLang']){
     ...
     {
      if($var!='updateLang'){
      $sql = "UPDATE ".LANGUAGE_TABLE." SET `".$var."`='".addslashes($value)."' WHERE LANGUAGE_ID='".$_POST['LANGUAGE_ID']."'";
      $result = mysql_query($sql)or die(mysql_error());
    ...
    // Update columns
    if($_POST['update']){
    	 $sql = "UPDATE ".COLUMNS_TABLE." SET active='" .$_POST['active']. "',name='" .$_POST['name']. "',description='" .$_POST['desc']. "',weight='" .$_POST['weight']. "' WHERE id='" .$_POST['id']. "'";
	 $result = mysql_query($sql) or die (mysql_error());
    }

    // Update Ticket Handling
    if($_POST['submit']){
	 $sql = "UPDATE ".CONFIG_TABLE." SET defaultAction='" .$_POST['defaultAction']. "' ";
	 $result = mysql_query($sql) or die (mysql_error());
    }
    ...
    case 'generallayout':
     ...
     $sqlpre = "UPDATE ".THEME_TABLE." SET ";
     if($_POST['submit']){

      // handle client logo
      if($_POST['dbClientLogo'] == $_POST['clientLogo']) {
       $sql_client ='';
       if($_POST['clientLogo'] == './images/logo2.jpg') {
        $sql_client = "clientlogoactive='0'";
       }
      }
      else {
       $sql_client = "clientlogo='".$_POST['clientLogo']."',clientlogoactive='1'";
      }

      // handle staff logo
      if($_POST['dbStaffLogo'] == $_POST['staffLogo']) {
       $sql_staff ='';
       if($_POST['staffLogo'] == '../images/logo1.png') {
        $sql_staff = "stafflogoactive='0'";
       }
      }
      else {
       $sql_staff = "stafflogo='".$_POST['staffLogo']."',stafflogoactive='1'";
      }
      ...
      if (file_exists($_POST['staffLogo'])) {   }
      else { 
       $errors['err']= $_POST['staffLogo']." doesn't exist! Make sure you install it!"; }
      if (file_exists($_POST['clientLogo'])) {   }
      else { 
       $errors['err']= $_POST['clientLogo']." doesn't exist! Make sure you install it!"; }
    ...
    case 'staff': // Was 'dept' - keep it ? 8/14/11 Scott
            ...
            case 'update':
                ...
                if($dept && $dept->getId()) {
                    if($dept->update($_POST,$errors))
                        $msg=translate('TEXT_DEPT_UPDATED_SUCCESSFULLY');
                    elseif(!$errors['err'])
...
switch($thistab){
    ...
    case 'api':
        ...
        $desc = translate('LABEL_PREFERENCES');
        $nav->addSubMenu(array('desc'=>$desc,'href'=>'admin.php?t=pref','iconclass'=>'preferences'));
        $desc = translate('LABEL_AUTHENTICATION');
        $nav->addSubMenu(array('desc'=>$desc,'href'=>'admin.php?t=auth','iconclass'=>'preferences'));
        $desc = translate('LABEL_ATTACHMENTS');
        $nav->addSubMenu(array('desc'=>$desc,'href'=>'admin.php?t=attach','iconclass'=>'attachment'));
        $desc = translate('LABEL_API');
    ...
    case 'dashboard':
        ...
        $desc = translate('LABEL_SYSTEM_LOGS');
        $nav->addSubMenu(array('desc'=>translate("LABEL_OVERVIEW"),'href'=>'admin.php?t=dashboard','iconclass'=>'syslogs'));
        $nav->addSubMenu(array('desc'=>translate("LABEL_REPORTS"),'href'=>'admin.php?t=reports','iconclass'=>'syslogs'));
    ...
    case 'reports':
        $nav->setTabActive('dashboard');
        $desc = translate('LABEL_SYSTEM_LOGS');
        $nav->addSubMenu(array('desc'=>translate("LABEL_DASHBOARD"),'href'=>'admin.php?t=dashboard','iconclass'=>'dashboard'));
        $nav->addSubMenu(array('desc'=>translate("LABEL_REPORTS"),'href'=>'admin.php?t=reports','iconclass'=>'syslogs'));
        $nav->addSubMenu(array('desc'=>$desc,'href'=>'admin.php?t=syslog','iconclass'=>'syslogs'));
    ...
    case 'syslog':
        $nav->setTabActive('syslog');
        $nav->addSubMenu(array('desc'=>translate("LABEL_DASHBOARD"),'href'=>'admin.php?t=dashboard','iconclass'=>'dashboard'));
        $nav->addSubMenu(array('desc'=>translate("LABEL_REPORTS"),'href'=>'admin.php?t=reports','iconclass'=>'syslogs'));        $desc = translate('LABEL_SYSTEM_LOGS'); 
        $nav->addSubMenu(array('desc'=>$desc,'href'=>'admin.php?t=syslog','iconclass'=>'syslogs'));
    ...
    case 'banlist':
        ...
        $nav->addSubMenu(array('desc'=>translate("EMAIL_ADDRESSES"),'href'=>'admin.php?t=email','iconclass'=>'emailSettings'));
        $nav->addSubMenu(array('desc'=>translate("ADD_NEW_EMAIL"),'href'=>'admin.php?t=email&a=new','iconclass'=>'newEmail'));
        $nav->addSubMenu(array('desc'=>translate("TEMPLATES"),'href'=>'admin.php?t=templates','title'=>'Email Templates','iconclass'=>'emailTemplates')); 
        $nav->addSubMenu(array('desc'=>translate("BANLIST"),'href'=>'admin.php?t=banlist','title'=>'Banned Email','iconclass'=>'banList')); 
        switch(strtolower($_REQUEST['t'])){
            case 'templates':
    ...
    case 'topics':
        ...
        $nav->addSubMenu(array('desc'=>translate("LABEL_HELP_TOPICS"),'href'=>'admin.php?t=topics','iconclass'=>'helpTopics'));
        $nav->addSubMenu(array('desc'=>translate("ADD_NEW_TOPIC"),'href'=>'admin.php?t=topics&a=new','iconclass'=>'newHelpTopic'));
    ...
    case 'layout':
        ...
        $nav->setTabActive('layout');
        $nav->addSubMenu(array('desc'=>translate("LABEL_GENERAL_SETTINGS"),'href'=>'admin.php?t=generalLayout','iconclass'=>''));
        $nav->addSubMenu(array('desc'=>translate("LABEL_STAFF"),'href'=>'admin.php?t=staffLayout','iconclass'=>''));
        $nav->addSubMenu(array('desc'=>translate("CLIENT"),'href'=>'admin.php?t=clientLayout','iconclass'=>''));
	$nav->addSubMenu(array('desc'=>translate("COLORS"),'href'=>'admin.php?t=colorLayout','iconclass'=>''));
        $nav->addSubMenu(array('desc'=>translate("LABEL_LANGUAGE"),'href'=>'admin.php?t=language','iconclass'=>'')); 

	default:
	$page='generallayout.inc.php';
	...
	$nav->addSubMenu(array('desc'=>translate("LABEL_DEPARTMENTS"),'href'=>'admin.php?t=depts','iconclass'=>'departments'));
        $nav->addSubMenu(array('desc'=>translate("ADD_DEPARTMENT"),'href'=>'admin.php?t=depts&a=new','iconclass'=>'newDepartment'));
        $nav->addSubMenu(array('desc'=>translate("LABEL_STAFF_MEMBERS"),'href'=>'admin.php?t=staff','iconclass'=>'users'));
        $nav->addSubMenu(array('desc'=>translate("ADD_STAFF"),'href'=>'admin.php?t=staff&a=new','iconclass'=>'newuser'));
        $nav->addSubMenu(array('desc'=>translate("USER_GROUPS"),'href'=>'admin.php?t=groups','iconclass'=>'groups'));
        $nav->addSubMenu(array('desc'=>translate("ADD_GROUP"),'href'=>'admin.php?t=groups&a=new','iconclass'=>'newgroup'));
        $page='';
        switch($thistab){
            case 'grp':
            case 'groups':
