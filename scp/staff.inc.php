...
/*************************************************************************
    ...
    $Id: staff.inc.php,v 1.1 2011/10/24 21:15:15 root Exp $
**********************************************************************/
...
if(!is_object($thisuser) || !$thisuser->getId() || !$thisuser->isValid()){
    $msg=(!$thisuser || !$thisuser->isValid())?translate('TEXT_AUTHENTICATION_REQUIRED'):translate('TEXT_SESSION_TIMED_OUT_DUE_TO_INACTIVITY_DUE_TO_INACTIVITY');
    staffLoginPage($msg);
    exit;
}
...
if(defined('THIS_VERSION') && strcasecmp($cfg->getVersion(),THIS_VERSION)) {
    ...
    $sysnotice=translate('TEXT_SYSTEM_IS_OFF_LINE');
    $sysnotice.=' <a href="admin.php?t=pref">'.translate('LABEL_ENABLE_SYSTEM_OFF_LINE').'</a>.';
}
